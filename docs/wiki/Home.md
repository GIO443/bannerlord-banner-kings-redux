...
Welcome. This is the player-facing handbook for **Banner Kings — Redux**, a maintenance fork of R-Vaccari's Banner Kings updated for **Bannerlord v1.4** with native **War Sails (NavalDLC)** Nord faction integration and the seven culture faiths seeded for the religion system. If you're looking for code internals and developer documentation, those live next to the code in the GitHub repository — these wiki pages are intentionally written for players.
...
Releases are tagged on the `1.9.x` line; the latest tag is the recommended build. Older tags remain available on the page if you specifically need to roll back, but no fixes are backported to them.
...
This **Redux fork** brings the mod current with Bannerlord v1.4 and adds a layer of native and compat work on top:
...
- **Bannerlord Living Economy integration** *(new in v1.9.0.0)* — BK's population and estate systems now run on top of [Bannerlord Living Economy](https://www.nexusmods.com/mountandblade2bannerlord/mods/10796), a **required dependency**. Settlements use its seven social classes; BK estates anchor onto its estate parcels; workshops are owned by Living Economy. One coherent economy instead of two simulations competing. See [Installing](Installing).
...
- **[Economy](Economy)** — village classes, town industries, estate specializations (incl. Growth investment mode), cluster fit, stagnation, food caravans, AI estate-policy ladder.
...
- **[Internal politics](Internal-Politics)** — flow graphs for vassal feuds (claim / usurp / revoke), clan promotion, and the AI's daily-tick climbing + ruler-revocation loops.
...
The 1.9.x line is the actively-maintained release line. Most reported issues fall into one of three buckets:
...
- **Religion-related weirdness.** v1.9.10.21 moved the religion subsystem to Banner Kings Content Expansion (BKCE). The **MCM → Performance → Enable Religion System (legacy)** toggle now defaults OFF — the seven faiths aren't seeded, the religion HUD widget is hidden, and the religion campaign behaviour is silent. Existing saves still load with their religion state preserved but inert. Flip the toggle back ON if you want the legacy in-BK religion system until BKCE ships.
